...
say("step 3/5 download GGUF (~13.3 GB)")
try:
...
# 256k context with q4_0 KV. Ornith is a MoE: only 10 of 40 layers are full
# attention (2 KV heads x 256 dim), so q4_0 KV is ~5 KB/token -> ~1.3 GB at
# 262144. Model 13.25 GB + KV 1.3 GB leaves ~1 GB for compute buffers; the
# ngl ladder backs off if VRAM allocation fails at load time.
